...
func connect() (*sql.DB, error) {
	...
	sqlStatement := `create table if not exists articles (id integer not null primary key autoincrement, title text, content text)`
...
func dbDeleteArticle(id string) error {
	query, err := db.Prepare("delete from article where id=?")
	defer query.Close()
